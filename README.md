...
A aplicação usa configuração por ambiente com estratégia híbrida de runtime: prioriza `import.meta.env` (quando disponível) e aplica fallback para `window.__ENV__` em deploys estáticos sem injeção Vite.
